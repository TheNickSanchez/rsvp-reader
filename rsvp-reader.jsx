...
function Quiz({ questions, onDone, wpm, wordCount, isCustom, textContent }) {
  ...
  const qs = aiQ || questions;

  useEffect(() => {
    if (isCustom && textContent && textContent.split(/\s+/).length > 30) {
      setLoading(true);
      (async () => {
        try {
          const r = await fetch("https://api.anthropic.com/v1/messages", {
            ...
            body: JSON.stringify({
              ...
              messages: [{ role: "user", content: `Generate 5 reading comprehension questions. Return ONLY valid JSON array, no markdown/backticks/preamble. Format: [{"q":"question","opts":["A","B","C","D"],"a":0}] where "a" is 0-indexed correct option. Test factual recall. Text:\n${textContent.slice(0, 3000)}` }]
            })
          });
          const d = await r.json();
          const t = d.content.map(i => i.text || "").join("\n").replace(/```json|```/g, "").trim();
          const p = JSON.parse(t);
          if (Array.isArray(p) && p.length >= 3) setAiQ(p.slice(0, 5));
          else setErr(true);
        } catch { setErr(true); }
        finally { setLoading(false); }
      })();
    }
  }, [isCustom, textContent]);

  if (isCustom && loading) return (
    <div style={{ textAlign: "center", animation: "fadeIn 0.5s ease" }}>
      <div style={{ fontFamily: MN, fontSize: "0.75rem", color: C.t3, letterSpacing: "0.15em", textTransform: "uppercase", marginBottom: "16px" }}>Generating Quiz</div>
      <div style={{ display: "flex", justifyContent: "center", gap: "8px", marginBottom: "24px" }}>
        {[0,1,2].map(i => <div key={i} style={{ width: 8, height: 8, borderRadius: "50%", background: C.ac, animation: `pulse 1.2s ease-in-out ${i*0.2}s infinite` }} />)}
      </div>
      <p style={{ fontSize: "0.85rem", color: C.t5, fontStyle: "italic" }}>Analyzing your text...</p>
    </div>
  );

  if ((isCustom && err) || !qs || qs.length === 0) return (
    <div style={{ textAlign: "center", animation: "fadeIn 0.5s ease" }}>
      <div style={{ fontSize: "1.1rem", color: C.t5, marginBottom: "16px" }}>{isCustom && err ? "Couldn't generate quiz" : "No quiz available"}</div>
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, color: C.tx, fontFamily: SF, display: "flex", flexDirection: "column", alignItems: "center", padding: 0, overflow: "hidden", position: "relative" }}>
      ...
      <main style={{ flex: 1, width: "100%", maxWidth: 900, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "0 32px", position: "relative", zIndex: 2, marginTop: -20 }}>
        ...
        {phase === "quiz" ? (
          <Quiz questions={td.questions} onDone={quizDone} wpm={train !== "off" ? liveWpm : wpm} wordCount={words.length} isCustom={sel === "Custom Text"} textContent={sel === "Custom Text" ? custom : td.text} />
        ) : intro ? (
          ...
          <div style={{ maxWidth: 700, margin: "0 auto" }}>
            ...
            {sel === "Custom Text" && (
              <div style={{ animation: "fadeIn 0.3s ease" }}>
                <textarea value={custom} onChange={e => setCustom(e.target.value)} placeholder="Paste your text here... (30+ words for AI-generated quiz)"
